...
export const runsApi = {
  ...
  /**
   * Saves a GPS point.
   * Returns { point, loopDetected } — caller checks loopDetected to show
   * territory capture immediately without a separate HTTP round-trip.
   */
  savePoint: async (
    runId: string,
    point: SavePointPayload
  ): Promise<SavePointResponse> => {
    const res = await apiClient.post<SavePointResponse>(
      `/runs/${runId}/points`,
      point
    );
    return res.data;
